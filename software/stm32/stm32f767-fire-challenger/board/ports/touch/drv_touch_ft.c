...
static void ft_init(struct rt_i2c_bus_device *i2c_bus)
{
    ...
    rt_pin_mode(BSP_TOUCH_RES_PIN, PIN_MODE_INPUT_PULLDOWN);
    rt_pin_mode(BSP_TOUCH_INT_PIN, PIN_MODE_INPUT_PULLDOWN);
...
static rt_bool_t ft_probe(struct rt_i2c_bus_device *i2c_bus)
{
    ...
    ft_i2c_bus = i2c_bus;
    /* GT9157 Chip identification register address is 0xBA */
    err = ft_read(ft_i2c_bus, 0xBA, (uint8_t *)&cid, 1);
    if (err < 0)
    {
...
{
    /* TouchScreen GT9157 Slave I2C address is 0x54
     * 0x54 << 1 = 0x2A
     */
